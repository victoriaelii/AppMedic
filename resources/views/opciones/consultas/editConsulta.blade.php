                        ...
                        <div class="mb-4">
                            ...
                            @foreach ($productos as $producto)
                                ...
                                <div class="flex items-center mb-2">
                                    <input type="checkbox" name="productos[{{ $producto->id }}][id]" value="{{ $producto->id }}" class="mr-2" onclick="toggleCantidad(this)" @if($cantidad) checked @endif>
                                    <label>{{ $producto->nombre }} - ${{ number_format($producto->precio, 2) }}</label>
                                    <input type="number" name="productos[{{ $producto->id }}][cantidad]" min="1" max="{{ $producto->cantidad }}" class="ml-4 p-2 border rounded-md w-20" data-producto-id="{{ $producto->id }}" @if(!$cantidad) disabled @endif value="{{ $cantidad }}">
                                </div>
                            @endforeach
                        </div>
    ...
    <script>
        function toggleCantidad(checkbox) {
            ...
            if (checkbox.checked) {
                ...
                cantidadInput.value = ''; // Deja el campo vacío cuando se selecciona el checkbox
            } else {
                cantidadInput.disabled = true;
                cantidadInput.value = ''; // Deja el campo vacío cuando se deselecciona
            }
        }
    </script>
</x-app-layout>
